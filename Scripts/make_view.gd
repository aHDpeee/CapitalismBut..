...
func addLetter(alf,col=0,row=0, pos=Vector3(0,1,0), rot=Vector3.ZERO):
		var l : RigidBody3D = lInstance.instantiate()
		l.letter = alf
		l.rotation = rot
		var line = (Vector3.RIGHT\
		 * Quaternion(Vector3.UP, -rot.y)\
		 #* Quaternion(Vector3.LEFT, rot.x)\
		 #* Quaternion(Vector3.FORWARD, rot.z)
		).normalized()
		l.position = pos + line*col*0.1
		...
		if move:
			...
			if 1:
				...
				if Controller.arePressed("c"):
					force += Vector3(-1,0,1)
				achi.apply_central_force(force.normalized()*50)
