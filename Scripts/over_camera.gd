...
func _input(event: InputEvent) -> void:
	if event is InputEventKey:
		if event.is_pressed():
			print('key')
			if Controller.inStack("/z") and status== "table":
				status = "window"; 
				if t && t.is_running(): t.stop();
				print("window")
				t = myTween()
				t.tween_property(self, "position", Vector3(0,defPos.y,0.8), 1.5)
				t.tween_property(self, "rotation_degrees", Vector3(0,0,0), 1.5)
				t.play()
